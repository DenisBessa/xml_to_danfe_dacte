...
const barcode = path.join(diretorioDeFontes, "code128.ttf");
...
export default (danfe, originalArgs, callback) => {
	const args = {
		...pdfDefaults,
		...originalArgs,
	};

	const emitente = danfe.getEmitente();
	const destinatario = danfe.getDestinatario();
	const transportador = danfe.getTransportador();
	const impostos = danfe.getImpostos();
	const volumes = danfe.getVolumes();
	const protocolo = danfe.getProtocolo();
	const itens = danfe.getItens();
	const fatura = danfe.getFatura();
	let alturaDoBlocoFaturaDuplicatas = 0;
	let duplicatas = danfe.getDuplicatas();
	const pdf = new Pdf({
		pdfVersion: "1.7",
		bufferPages: true,
		margin: 0,
	...
	function desenharPagina() {
		...
		const larguraDaDuplicataComMargem =
			larguraDaDuplicata - margemEsquerdaDaDuplicata - 1;

		duplicatas.forEach((linha, indiceDaLinha) => {
			let x1;
			let x2;
			let y;
			...
			linha.forEach((duplicata, indiceDaDuplicata) => {
				...
				titulo(
					"VALOR",
					x1 + margemEsquerdaDaDuplicata,
					y + margemDeCimaDaDuplicata + 2 * args.tamanhoDaFonteDoTitulo,
					larguraDaDuplicataComMargem,
					"left",
				);
				titulo(
		...
		const segundaLinhaDoQuadroDeItens = alturaInicialDoQuadroDeItens + 14.2;
		...
		const temLogotipo = emitente.getLogotipo();
		const identificacaoDoEmitenteY = temLogotipo ? 78 : 84;
		const identificacaoDoEmitenteX = temLogotipo ? 67 : 1.5;
		const identificacaoDoEmitenteLargura = temLogotipo ? 172 : 237; //
		const identificacaoDoEmitenteFonte = temLogotipo ? 0 : 1.5;

		if (temLogotipo) {
			const caminhoDoLogotipo = emitente.getLogotipo();
			pdf.image(
				caminhoDoLogotipo,
		...
		const formularioDeSeguranca = danfe.getFormularioDeSeguranca();
		...
		const informacoesComplementares = [danfe.getInformacoesComplementares()];
		...
		const codigoDeBarrasCodificado = danfe.getChaveDeAcesso();
		...
		const alturaDosTitulosDosItens = alturaDaSecaoDosItens + adicional_Itens_y;
		...
		const alturaDosTitulosDoSetimoBloco = alturaDaSecaoDoSetimoBloco + 9;
	...
	for (const item of itens) {
		function renderizarLinha(pdf) {
			...
			const infoAdicional = item.getInformacoesAdicionais();
