...
exports.default = (function (danfe, originalArgs, callback) {
    var args = __assign(__assign({}, pdfDefaults), originalArgs);
    var emitente = danfe.getEmitente();
    var destinatario = danfe.getDestinatario();
    var transportador = danfe.getTransportador();
    var impostos = danfe.getImpostos();
    var volumes = danfe.getVolumes();
    var protocolo = danfe.getProtocolo();
    var itens = danfe.getItens();
    var fatura = danfe.getFatura();
    var alturaDoBlocoFaturaDuplicatas = 0;
    var duplicatas = danfe.getDuplicatas();
    ...
    var pdfTemporario = new pdfkit_1.default({
        //TODO: Verificar se a criação deste pdf temporario
        //      está causando problemas de performance
        margin: 0,
        size: [args.larguraDaPagina, args.alturaDaPagina],
    });
    var deveExibirQuadroDeCalculoDoIssqn = danfe.getValorTotalDosServicos() ||
    ...
    function normal(inputString, x, y, largura, alinhamento, tamanho, _pdf) {
    ...
    function italico(inputString, x, y, largura, alinhamento, tamanho) {
        var string = inputString || "";
        pdf
            .font("italico")
            .fillColor(args.corDoTitulo)
    ...
    function negrito(inputString, x, y, largura, alinhamento, tamanho) {
        var string = inputString || "";
        pdf
            .font("negrito")
            .fillColor(args.corDoTitulo)
    ...
    function campo(inputString, x, y, largura, alinhamento, tamanho) {
        var string = inputString || "";
        pdf
            .font("negrito")
    ...
    function desenharPagina() {
        ...
        var alturaInicialDoQuadroDeDuplicatas = alturaInicialDoQuadroDeItens;
        var alturaDeLinhaDeDuplicata = 23;
        var duplicatasPorLinha = 8;
        var larguraDaDuplicata = larguraDoFormulario / duplicatasPorLinha;
        var linhasDeDuplicatas = 0;
        var adicional_Itens_y = 22;
        if (fatura) {
            adicional_Itens_y -= 9;
        ...
        duplicatas = duplicatas.slice(0, duplicatasPorLinha);
        var margemDeCimaDaDuplicata = 2;
        var margemEsquerdaDaDuplicata = 2;
        var larguraDaDuplicataComMargem = larguraDaDuplicata - margemEsquerdaDaDuplicata - 1;
        duplicatas.forEach(function (linha, indiceDaLinha) {
            ...
            linha.forEach(function (duplicata, indiceDaDuplicata) {
                ...
                titulo(duplicata._numero, x1 + margemEsquerdaDaDuplicata, y + margemDeCimaDaDuplicata, larguraDaDuplicataComMargem, "right");
                ...
                titulo(duplicata._vencimento, x1 + margemEsquerdaDaDuplicata, y + margemDeCimaDaDuplicata + args.tamanhoDaFonteDoTitulo, larguraDaDuplicataComMargem, "right");
                ...
                titulo(duplicata._valor, x1 + margemEsquerdaDaDuplicata, y + margemDeCimaDaDuplicata + 2 * args.tamanhoDaFonteDoTitulo, larguraDaDuplicataComMargem, "right");
            });
        ...
        var segundaLinhaDoQuadroDeItens = alturaInicialDoQuadroDeItens + 14.2;
        ...
        normal([
            ...
            .join(" ")
            .toUpperCase(), 1.5, 3, 472.5, "justify", 6.9);
        ...
        var identificacaoDoEmitenteY = temLogotipo ? 78 : 84;
        var identificacaoDoEmitenteX = temLogotipo ? 67 : 1.5;
        var identificacaoDoEmitenteLargura = temLogotipo ? 172 : 237; //
        var identificacaoDoEmitenteFonte = temLogotipo ? 0 : 1.5;
        if (temLogotipo) {
        ...
        if (emitente.getTelefone()) {
            ...
            normal("Telefone: ".concat(emitente.getTelefoneFormatado()), identificacaoDoEmitenteX, pdf.y - margemTopo + 2, identificacaoDoEmitenteLargura, "center", 6 + identificacaoDoEmitenteFonte);
        }
        if (emitente.getEmail()) {
            normal("Email: ".concat(emitente.getEmail()), identificacaoDoEmitenteX, pdf.y - margemTopo + (jaAdicionouEspacamento ? 0 : 2), identificacaoDoEmitenteLargura, "center", 6 + identificacaoDoEmitenteFonte);
        }
        ...
        if (protocolo.getFormatacao()) {
            ...
            pdf
                .font("codigoDeBarras")
                .fontSize(18.5)
                .text(formularioDeSeguranca.getDadosDaNfe(), 341.5, 131.5, {
                align: "center",
                width: 250,
            });
        }
        ...
        var codigoDeBarrasCodificado = danfe.getChaveDeAcesso();
        ...
        if (fatura) {
            var dadosDaFatura = [];
            if (fatura.getNumero()) {
                dadosDaFatura.push("N\u00DAMERO: ".concat(fatura.getNumero()));
            }
            if (fatura.getNumero()) {
                dadosDaFatura.push("VALOR ORIGINAL: ".concat(fatura.getValorOriginalFormatado()));
            }
            if (fatura.getNumero()) {
                dadosDaFatura.push("VALOR DO DESCONTO: ".concat(fatura.getValorDoDescontoFormatado()));
            }
            if (fatura.getNumero()) {
                dadosDaFatura.push("VALOR L\u00CDQUIDO: ".concat(fatura.getValorLiquidoFormatado()));
            }
            if (fatura.getFormaDePagamento()) {
                dadosDaFatura.push("PAGAMENTO: ".concat(fatura.getFormaDePagamento()));
            }
        ...
        var alturaDosTitulosDosItens = alturaDaSecaoDosItens + adicional_Itens_y;
        // QUINTO BLOCO - LISTA DE PRODUTOS
        ...
        var alturaDosTitulosDoSetimoBloco = alturaDaSecaoDoSetimoBloco + 9;
        // SÉTIMO BLOCO
    ...
    var yInicialDosItens = 406 + alturaDoBlocoFaturaDuplicatas;
    var yDoItemAtual = yInicialDosItens;
    var alturaAtual = 0;
    var _loop_1 = function (item) {
        function renderizarLinha(pdf) {
            ...
            var infoAdicional = item.getInformacoesAdicionais();
            if (item.getInformacoesAdicionais()) {
                descricao += "\n".concat(infoAdicional);
            }
            normal(descricao, 55.5, yDoItemAtual, 178, "justify", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._ncmSh, 235.5, yDoItemAtual, 32.5, "center", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._oCst, 270, yDoItemAtual, 21, "center", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._cfop, 293.5, yDoItemAtual, 21, "center", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._unidade, 315, yDoItemAtual, 16.5, "center", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._quantidade, 332.5, yDoItemAtual, 37.5, "right", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._valorUnitario, 372, yDoItemAtual, 32.5, "right", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._valorTotal, 407, yDoItemAtual, 31, "right", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._baseDeCalculoDoIcms, 440, yDoItemAtual, 32.5, "right", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._baseDeCalculoDoIcms, 440, yDoItemAtual, 32.5, "right", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._valorDoIcms, 474.5, yDoItemAtual, 32, "right", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._valorDoIpi, 508.5, yDoItemAtual, 26, "right", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._aliquotaDoIcms, 536, yDoItemAtual + 0.65, 19, "center", args.tamanhoDaFonteDosItens - 1, pdf);
            maiorY = Math.max(maiorY, pdf.y);
            normal(item._aliquotaDoIpi, 556.5, yDoItemAtual, 29, "center", args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
    ...
    for (var i = paginas.start; i < paginas.start + paginas.count; i++) {
        pdf.switchToPage(i);
        italico("FOLHA ".concat(i + 1, "/").concat(numeroDePaginas), 241.2, 141.2, 98.5, "center", 8);
    }
    pdf.flushPages();
    pdf.end();
    callback(null, pdf);
